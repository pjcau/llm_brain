...
| 2026-09-19 | **llm_brain runs on a VPS (Hetzner, ~€4–5/month)** with a protected public HTTPS endpoint; **the apps have no location constraint** (AWS, VPS, home). Performance is not a criterion: model time dominates. Re-checked with the Rust footprint: cost parity with Lightsail $5, decision kept on headroom/simplicity; Lightsail is a penalty-free fallback if the apps land on AWS. | [Hosting](./analysis/hosting-costs.md#with-everything-in-rust-is-the-vps-still-ahead-of-aws) |
