...
| 2026-09-19 | **llm_brain is a single, standalone backend** that centralizes every LLM request. agent-orchestrator **uses it** as a client, like the apps. | own repo; `usage.py`, `cache.py`, `openrouter.py`, `evaluator.py` are **copied** from agent-orchestrator, not depended on — [What to reuse](./analysis/agent-orchestrator.md) |
...
| 2026-09-19 | `llm_brain` repo structure: `src/llm_brain/` + `bench/` + `config/` + `website/`. | [Stack](./analysis/stack.md#repo-structure-decided-on-2026-09-19) |
